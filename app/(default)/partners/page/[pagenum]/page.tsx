...
export default async function Partner({ params }: { params: { pagenum: string } }) {
  const pageNo = parseInt(params.pagenum) ?? 1

  const blogPosts = await getPartnerPostsPaged(parseInt(params.pagenum))
            ...
            <div className='max-w-sm mx-auto md:max-w-none'>
              ...
              <div className='grid gap-12 md:grid-cols-3 md:gap-x-6 md:gap-y-8 items-start'>
                {posts.map((post, postIndex) => (
                  <div key={post.uid}>{post.data.title}</div>
